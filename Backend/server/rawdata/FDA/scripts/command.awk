...
{gsub(/\*/, "", $4)}
# removes "(TO MAINTAIN ..)" and '.' if it follows
{gsub(/(TO MAINTAIN.*)\.?,/, "", $4)}
# replaces open parens with ', ' in ingredients column
{gsub(/ \(/, ", ", $4)}
# replaces ")" and ")," with ',' in ingredients column
{gsub(/\),?/, ",", $4)}
# deletes "." if followed by a blank or control char in ingredients column
{gsub(/\.[:space:]/, "", $4)}
# deletes the phrase "contains 2% or less of: " and "contains less than 2% " from ingredients
{gsub(/CONTAINS.*[:%] /, "", $4)}
# deletes everything from ',' to a left bracket, replaces with ','
{gsub(/,.*\[/, ", ", $4)}
# deletes right brackets
{gsub(/\]/, "", $4)}
# deletes "VITAMIN --"
{gsub(/ VITAMIN.*,/, "", $4)}
